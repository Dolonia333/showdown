...
def visualize_prediction(
  ...
  try:
    ...
    if run_id:
      vis_dir = os.path.join(base_dir, 'results', run_id, model_name, 'visualizations')
    else:
    ...
    output_filename = (
      f'rec{recording_id}_item{item_id}_{image_filename}_{urllib.parse.quote_plus(instruction)}.png'
    )
    if is_in_bbox:
